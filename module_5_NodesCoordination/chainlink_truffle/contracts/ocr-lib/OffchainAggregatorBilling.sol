...
contract OffchainAggregatorBilling is Owned {
  ...
  function fundOCR(uint amount, address requester, address OCR_contract)
  internal
  {
    require(s_linkToken.transferFrom(requester, OCR_contract, amount), "insufficient funds");
  }

  function refundRequester(uint amount, address addr)
  internal
  {
    if(amount > 0)
      require(s_linkToken.transfer(addr, amount), "insufficient funds");
  }
}
