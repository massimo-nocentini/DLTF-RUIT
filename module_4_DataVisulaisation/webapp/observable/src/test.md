...
<div id="graph-container">
  ...
  <div id="side-panel">
    ...
    <div class="navigation-header">
      ...
      <div class="current-node">Current: <span id="current-node-id">37028167</span></div>
    </div>
...
const navigationState = {
  history: ['37028167'], // Start with node 15
  currentIndex: 0,
...
async function fetchGraphData(nodeId = '37028167') {
...
async function fetchPlotData(nodeAddress, dataType) {
  try {
    ...
    if (dataType === 'liveness') {
      ...
      url = `${DRE_API_BASE}/sc-liveness?address=${nodeAddress}&sample=0`;
    }
...
async function renderBarPlot(containerId, data, title) {
  ...
  // Larger margins and dimensions for better readability
  ...
  // X-axis with better label spacing
  svg.append("g")
...
async function renderGraph(nodeId = '37028167') {
  // Get the actual container dimensions for full screen
  ...
  try {
    ...
    nodes.forEach(node => {
      if (node.id == nodeId) {
        ...
        node.x = width/2 + Math.random()*400-200;
        node.y = height/2 + Math.random()*400-200;
      }
    });
